...
class RecRunner:
    ...
    @staticmethod
    def get_base_recommenders(self):
        return {
    ...
    @staticmethod
    def get_final_recommenders(self):
        return {
            "geocat": self.geocat,
            "none":None
        }
    def load_base(self):
        ...
        #Train load
        self.data_checkin_train = pickle.load(open(self.data_directory+"checkin/train/"+CITY+".pickle","rb"))
        ...
        # Cat Hierarchy load
        self.dict_alias_title,self.category_tree,self.dict_alias_depth=cat_utils.cat_structs(self.data_directory+"categories.json")
        ...
        # poi neighbors load
        self.poi_neighbors=pickle.load(open(self.data_directory+"neighbor/"+CITY+".pickle","rb"))
    ...
    def run_usg(self,U,S,G,uid,alpha,beta):
        if uid in self.ground_truth:
            ...
            overall_scores= list(reversed(np.sort(overall_scores)))[:self.base_rec_list_size]
    ...
    def usg(self):
        ...
        print("Running usg")
        futures =[executor.submit(self.run_usg,U,S,G,uid,alpha,beta) for uid in all_uids]
        results = [future.result() for future in futures]
        print("usg terminated")
        # dview.map_sync(run_usg,range(user_num))
        
        result_out = open(self.data_directory+"result/reclist/"+self.city+"_sigir11_top_" + str(top_k) + ".json", 'w')
        for json_string_result in results:
            result_out.write(json_string_result)
        result_out.close()
        print("Saved Rec List in "+self.data_directory+"result/reclist/"+self.city+"_sigir11_top_" + str(top_k) + ".json")

    def mostpopularnp(self,uid):
    ...
    def run_geocat(self,uid):
        if uid in self.ground_truth:
            pois_score=self.mostpopularnp(uid)
            predicted=list(reversed(np.argsort(pois_score)))[0:self.base_rec_list_size]
            overall_scores=list(reversed(np.sort(pois_score)))[0:self.base_rec_list_size]
    ...
    def geocat(self):
        ...
        futures =[executor.submit(self.run_geocat,uid) for uid in self.all_uids]
        results = [future.result() for future in futures]

        result_out = open(self.data_directory+"result/reclist/"+self.city+"_geocat_" + str(self.base_rec_list_size) + ".json", 'w')
        for json_string_result in results:
            result_out.write(json_string_result)
        result_out.close()